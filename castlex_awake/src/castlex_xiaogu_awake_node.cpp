...
int main(int argc, char* argv[])
{
	...
	*/
	...
	string path2 = ros::package::getPath("castlex_awake");
	//printf(path.data());
	string path3 = "/res/ivw/wakeupresource.jet";
	string my_ssb_param = path1 + path2 + path3;
	string bingo_music_path = "/res/music/zaidezhuren.wav";
	bingo_music_path = string("play ")+path2 + bingo_music_path;

	ros::NodeHandle nh("~");    //用于launch文件传递参数
	nh.param("lgi_param", my_lgi_param, std::string("appid = 551d49a3,work_dir = ."));    //从launch文件获取参数
