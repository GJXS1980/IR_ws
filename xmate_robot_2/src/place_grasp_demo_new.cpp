...
{ 
    ...
    int arcodeid = 0;
    int push_id = 0;
    double move_dev_LR = 0.05;
    double move_dev_FB = 0.05;
    ...
    Robot_Interface.Jog0_Robot_Moment = 8;
    ...
    ros::Subscriber aruco_marker_sub1 = n.subscribe("/aruco_single1/pose", 1, &HG_AI_Robot::Get_UR_Pose, &Robot_Interface);
    ...
    try{
        ...
        {   
            ...
            if(i == 4)
            {
                ...
                while(no_data_time < 6){
                    no_data_time +=1;
                    if(no_data_time>5 && !move_backward){
                        Robot_Interface.Robot_MoveL(0.0,0.0,0.01,robot);
                        Robot_Interface.Robot_MoveJ(Robot_Interface.grasp_identify_pose,robot);
             ...
             if(msg && BJJ_Grasp == false)
	     {
            ...
            if(move_left && move_right && move_forward && move_backward)
            {
                //抓取中间位姿
                Robot_Interface.Robot_MoveJ(Robot_Interface.grasp_fixed_middle_pose,robot);
       ...
       //清除之前识别到的无用数据
        ...
        while(Robot_Interface.Ur_Pose[0][0]==0.0){
            ...
            if(msg){
                ...
                no_data_time +=1;
                if(no_data_time>5 && !move_forward){
                    Robot_Interface.Robot_MoveL(0.0,0.05,0.0,robot);
                    move_forward = true;
                    no_data_time = 0;
        ...
        if(close_state8){
         ...
         /*-----判断往左往右-----*/
        if (move_forward && !move_backward && !move_right && !move_left)
        {
            std::cout<<"move forward"<<std::endl;
            Robot_Interface.Robot_MoveL(0.0,Robot_Interface.Move_Deviation1,0.0,robot);
            move_left = move_right = move_forward = move_backward = false;
        }else if(move_forward && move_backward && !move_right && !move_left)
        {
            std::cout<<"move backward"<<std::endl;
            Robot_Interface.Robot_MoveL(0.0,-Robot_Interface.Move_Deviation1,0.0,robot);
            move_left = move_right = move_forward = move_backward = false;
