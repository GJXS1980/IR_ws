...
string FILE_path("/bin/voice_control.bnf");
...
string xml_path("/params/voice_control.xml");
...
{
	printf(path1.data());
	ros::init(argc, argv, "castlex_xiaogu_control_order_node");    //初始化节点，向节点管理器注册
	ros::NodeHandle n;
	ros::Subscriber sub = n.subscribe("/voice/castlex_xiaogu_awake_topic", 1, orderCallback);
