...
void asr_Process()
{
	...
	const char* login_params = appid.data();//"appid = 5b6d44ed, work_dir = .";
...
int main(int argc, char** argv)
{
	...
	ros::init(argc, argv, "castlex_xiaogu_asr_node");    //初始化节点，向节点管理器注册
	ros::NodeHandle n;
	ros::Subscriber sub = n.subscribe("/voice/castlex_xiaogu_awake_topic", 1, asrCallback);	//	订阅语音唤醒话题
	//ros::Subscriber sub_tts = n.subscribe("/voice/castlex_xiaogu_tts_topic", 1, ttsCallback);	//	订阅语音合成话题

	ros::NodeHandle nh("~");    //用于launch文件传递参数
	nh.param("appid", appid, std::string("appid = 551d49a3, work_dir = ."));    //从launch文件获取参数
	...
	ros::Publisher pub = n.advertise<std_msgs::String>("/voice/castlex_xiaogu_asr_topic", 3);	//	发布语音识别话题
