...
int main(int argc, char* argv[])
{
	...
	bool tts_flag = false;
	ros::init(argc, argv, "castlex_xiaogu_tts_node");    //初始化节点，节点名为"castle_xf_tts_node"
	ros::NodeHandle n;
	ros::Subscriber sub = n.subscribe("/voice/castlex_xiaogu_nlu_topic", 3, xfCallBack);    //	订阅语义理解话题
	//ros::Subscriber sub2 = n.subscribe("/voice/castle_order_topic",orderCallBack);    //命令词识别失败时进行语音交互
	pub = n.advertise<std_msgs::Int32>("/voice/castlex_xiaogu_tts_topic", 1); 		//	发布语音合成话题
	asr_pub = n.advertise<std_msgs::Int32>("/voice/castlex_xiaogu_awake_topic", 1);  //	发布语音唤醒数据话题
