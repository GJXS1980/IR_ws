...
string FILE_path("/bin/castlex_auto_cmd.bnf");
...
string xml_path("/params/castlex_auto.xml");
...
int iatalk()
{
	const char *login_config    = "appid = 551d49a3"; //登录参数 有效期至 2019-09-18
	UserData asr_data; 
	int ret                     = 0 ;
	char c;
...
{
	printf(path1.data());
	ros::init(argc, argv, "castlex_xiaogu_auto_order_node");    //初始化节点，向节点管理器注册
	ros::NodeHandle n;
	ros::Subscriber sub = n.subscribe("/voice/castlex_xiaogu_awake_topic", 1, orderCallback);	//	订阅语音唤醒话题
	...
	ros::Publisher pub = n.advertise<std_msgs::String>("/voice/castlex_xiaogu_ocw_topic", 3);	// 发布离线命令词识别结果话题
	ros::Publisher cmd_pub = n.advertise<std_msgs::Int32>("/voice/castlex_xiaogu_cmd_topic", 1);	//	识别离线命令词成功的flag话题
